...
function Chatforres() {
    ...
    useEffect(()=>{
            async function fetchData(){
                let respone = await Axios.get(`http://localhost:3216/restaurant/chatforresreact?idcus=${id}&idres=${Cookies.get('idres')}`);
    ...
    useEffect(()=>{
                ...
                socket.on('server-sent-chat' , (data)=>{
                    console.log(data);
                    if(data.IdRestaurant === Cookies.get('idres') && data.IdCustomer === id){
                        setChat(data)
                      }
                })
    },[chat._id,id])
...
function enterChat(event){
    if(event.keyCode === 13){
        socket.emit('client-sent-chat' , {idsent : Cookies.get('idres') , IdCustomer : id ,IdRestaurant : Cookies.get('idres') , chatcontent : event.target.value , time : '12/12/2020', Name : Cookies.get('nameres') });
        event.target.value = '';
    }
    ...
    <div className="col-md-8">
      <div className="settings-tray" style={{height: '80px'}}>
        <div className="friend-drawer no-gutters friend-grey" style={{height: '80px'}}>
          <div className="text">
            <h6 id="name" style={{color: 'black'}}>{chat.ResName}</h6>
            <h6 style={{color: 'black'}}>ID : <span id="resid">{chat.IdRestaurant}</span> </h6>
